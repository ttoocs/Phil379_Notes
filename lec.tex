...
\item ( 
\item ) 
\item ' %No idea what this thing does.
\item - %Nor this really
...
G = $\exsits v _______$ or $G = \forall v _____________.$
\end{definition}

\begin{definition}[Model]
A model $M$ (interpretation) of a language $L$ is $\{|M| v\}$ Where $|M|$ is a non-empty set and $v$ is a valuation function that assigns values (extensions/denotations) to the mebers of $L$ in such a way that \begin{itemize}
...
$M \vDash F^n (t_1,\dots,t_n)$ iff $<M(t_1),\dots,M(t_n)> \in M(F^n)$.
\end{definition}
